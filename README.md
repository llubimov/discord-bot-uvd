...
**Повышения** — рапорты приходят вебхуком (Google Form) или создаются вручную. В каналах повышений у сообщения кнопки «Одобрить» / «Отклонить». При одобрении снимаются старые звания, выдаётся новая роль по маппингу. Аудит (принят/повышен/уволен) только в Google Form по `AUDIT_FORM_URL`.
...
Доступны только пользователям с ролью выше роли бота: `/ping`, `/diag`, `/diag_clean_orphans`, `/clear_firing`. Про текущие глюки с частью команд см. `STATUS.md`.
