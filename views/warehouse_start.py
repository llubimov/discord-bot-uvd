    ...
    async def cart_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            for cat, cat_items in by_category.items():
                cat_text = ""
                for it in cat_items:
                    cat_text += f"• {it['item']} — **{it['quantity']}** шт\n"
                    
                    if cat == "оружие":
                        weapon_count += it['quantity']
                    elif cat == "бронежилеты":
                        armor_count += it['quantity']
                    elif cat == "медикаменты":
                        meds_count += it['quantity']
            ...
            logger.error(f"Ошибка в корзине: {e}")
            from views.message_texts import ErrorMessages
